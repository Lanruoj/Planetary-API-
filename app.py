...
def db_seed():
    ...
    test_user = User(first_name='Tane',
                    last_name='Kaio',
                    email='test@test.com',
                    password='1234')
...
def login():
    ...
    if test:
        ...
        return jsonify(message='Login successful!', acces_token=access_token)
    else:
...
@app.route('/planets/update', methods=['PUT'])
@jwt_required()
def update_planets():
    # FIND PLANET BY planet_id
    planet_id = int(request.form['planet_id'])
    planet = Planet.query.filter_by(planet_id=planet_id).first()
    # IF PLANET EXISTS
    if planet:
        # UPDATE VALUES FOR planet
        planet.planet_name = request.form['planet_name']
        planet.planet_type = request.form['planet_type']
        planet.ome_star = request.form['home_star']
        planet.mass = float(request.form['mass'])
        planet.radius = float(request.form['radius'])
        planet.distance = float(request.form['distance'])
        # COMMIT CHANGES, DON'T NEED .add FOR UPDATE
        db.session.commit()
        # 202 = ACCEPTED
...
def delete_planet(planet_id: int):
    # CHECK IF PLANET EXISTS WITH planet_id ARGUMENT
    planet = Planet.query.filter_by(planet_id=planet_id).first()
    if planet:
